...
open class AudioNavigator: Navigator, Configurable, AudioSessionUser, Loggable {
    ...
    public class CustomPlayer: AVPlayer { }

    public lazy var player: CustomPlayer = {
        ...
        NotificationCenter.default.addObserver(forName: .AVPlayerItemDidPlayToEndTime, object: nil, queue: .main) { [weak self, weak player] notification in
            guard
                let self = self,
    ...
    private func shouldPlayNextResource(completion: @escaping (Bool) -> Void) {
        ...
        makePlaybackInfo { [weak self] info in
